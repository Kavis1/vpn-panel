...
const SubscriptionDetailsPage: React.FC = () => {
  ...
  const updateWhitelistMutation = useMutation(
    (ips: string[]) => updateIpWhitelist(id!, ips),
    {
      onSuccess: () => {
  ...
  const toggleIpRestrictionsMutation = useMutation(
    (enabled: boolean) => updateSubscription(id!, { ip_restriction: enabled ? 'enabled' : 'disabled' }),
    {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ['subscription', id] });
      }
  ...
  const handleToggleIpRestrictions = async (enabled: boolean) => {
    if (!id) return;
    try {
      await updateSubscription(id, { ip_restriction: enabled ? 'enabled' : 'disabled' });
      await queryClient.invalidateQueries({ queryKey: ['subscription', id] });
      setSnackbar({
        open: true,
        ...
        {isLoadingLogs ? (
          ...
          (() => {
            const formattedLogs = (logsData?.data || []).map(log => ({
              ...
              status: log.status as 'error' | 'success' | 'info' | 'warning' || 'info',
            ...
            return (
              <SubscriptionLogsTab
                ...
                pagination={{
                  ...
                  totalPages: Math.ceil((logsData?.total || 0) / logPagination.limit)
                }}
                filters={{
        ...
        {subscriptionData && (
          <SubscriptionSecurityTab
            subscription={{
              ...
              ip_whitelist: Array.isArray(subscriptionData.subscription.ip_whitelist)
                ? subscriptionData.subscription.ip_whitelist.map(ip => typeof ip === 'string' ? ip : (ip as IpAddress).address) as string[]
                : [],
              ip_blacklist: Array.isArray(subscriptionData.subscription.ip_blacklist)
                ? subscriptionData.subscription.ip_blacklist.map(ip => typeof ip === 'string' ? ip : (ip as IpAddress).address) as string[]
                : [],
              enforce_ip_restrictions: subscriptionData.subscription.ip_restriction === 'enabled'
            }}
