...
const SubscriptionsPage: React.FC = () => {
  ...
  const renewSubscriptionMutation = useMutation(renewSubscription, {
    onSuccess: () => {
      queryClient.invalidateQueries(['subscriptions']);
    },
  });
  
  const toggleStatusMutation = useMutation(toggleSubscriptionStatus, {
    onSuccess: () => {
      queryClient.invalidateQueries(['subscriptions']);
    },
  });
                    ...
                    <TableCell>
                      <Typography variant="body2">
                        {subscription.expire_date 
                          ? format(new Date(subscription.expire_date), 'dd.MM.yyyy', { locale: ru })
                          : 'Бессрочно'}
                      </Typography>
                      <Typography variant="caption" color="textSecondary">
                        {subscription.expire_date && (
                          isExpired 
                            ? `Истекла ${formatDistanceToNow(new Date(subscription.expire_date), { 
                                locale: ru, 
                                addSuffix: true 
                              })}`
                            : `Истекает ${formatDistanceToNow(new Date(subscription.expire_date), { 
                                locale: ru, 
                                addSuffix: true 
                              })}`
                        )}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <DialogActions>
            ...
            <Button 
              ...
              color="primary" 
              variant="contained"
              disabled={createSubscriptionMutation.isLoading || updateSubscriptionMutation.isLoading}
            >
              {(createSubscriptionMutation.isLoading || updateSubscriptionMutation.isLoading) ? (
